...
public interface PassengerRepository extends CrudRepository<Passenger,String>{
    @Query(value = "SELECT DISTINCT * FROM reservationsystem.passenger p, reservationsystem.reservation r WHERE p.id = r.id AND r.reservation_number = :reservationNumber",
            nativeQuery = true)
